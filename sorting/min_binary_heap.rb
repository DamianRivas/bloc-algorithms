...
class MinBinaryHeap
  ...
  def delete(root, data)
    ...
    while heap_array[i]
      current_node = heap_array[i]
      index = i if heap_array[i].data == data
    ...
    if last_is_left_node
      parent_of_last.left = nil if parent_of_last
    else
      parent_of_last.right = nil if parent_of_last
    end
  ...
  def printf(queue = Array.new())
    ...
    while queue[0]
      ...
      print "#{current_node.data}: #{current_node.rating}\n"
